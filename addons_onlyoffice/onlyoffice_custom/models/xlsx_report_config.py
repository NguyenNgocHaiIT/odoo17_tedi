...
class XlsxReportConfig(models.Model):
    ...
    report_xlsx_jinja_template = fields.Binary(
        related='attachment_id.datas',
        required=True,
        readonly=True,
        string="Report XLSX Jinja Template")
    ...
    def _compute_attachment_binary(self):
        for record in self:
            if record.attachment_ids:
                record.report_xlsx_template = record.attachment_ids[0].datas
                record.report_xlsx_template_filename = record.attachment_ids[0].name
            else:
