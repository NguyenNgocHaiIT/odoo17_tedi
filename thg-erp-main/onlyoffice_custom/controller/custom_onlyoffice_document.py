...
class OnlyofficeDocuments_custom(Onlyoffice_Connector):
    ...
    def render_editor(self, attachment_id, access_token=None):
        ...
        can_read = attachment.sudo().check_access_rights("read", raise_exception=False) and file_utils.can_view(filename)
        can_write = attachment.sudo().check_access_rights("write", raise_exception=False) and file_utils.can_edit(filename)
    ...
    def prepare_editor_values(self, attachment, access_token, can_write):
        ...
        can_write = False
        if document_share:
            rolls, users, public = [rec.role_access for rec in document_share.sudo().role_access_ids], document_share.user_ids, document_share.public_access
            roll_access = {
                "edit": "edit" in rolls,
            ...
            if not public:
                if request.env.user != attachment.sudo().create_uid:
                    if request.env.user not in users:
