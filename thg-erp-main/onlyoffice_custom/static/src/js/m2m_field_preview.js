...
class Many2ManyBinaryPreviewField extends Many2ManyBinaryField {
    ...
    openOnlyoffice(file, readonly = false) {
        const baseUrl = window.location.origin;

        if (this.isImage(file)) {
            this.dialogService.add(M2MImagePreviewDialog, {
                imageUrl: this.getUrl(file.id),
            });
            return;
        }

        const url = readonly
            ? `${baseUrl}/onlyoffice/editor/${file.id}?readonly=1`
            : `${baseUrl}/onlyoffice/editor/${file.id}`;
    ...
    onFileClicked(file, ev) {
        ...
        this.openOnlyoffice(file, readonly);
    }
