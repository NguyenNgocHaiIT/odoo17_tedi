...
class PhanPhat(models.TransientModel):
    ...
    nguoi_xu_ly_chinh = fields.Many2many(
        'res.users',
        'office_document_internal_phan_phat_nguoi_xu_ly_chinh_rel',
        'phanphat_id', 'user_id',
        string='Người xử lý chính')
    nguoi_dong_xu_ly = fields.Many2many(
        'res.users',
        'office_document_internal_phan_phat_nguoi_dong_xu_ly_rel',
        'phanphat_id', 'user_id',
    ...
    def phan_phat(self):
        ...
        doc = self.env['office.document.internal'].browse(doc_id)
        if not doc.exists():
            return
        ...
        for user in self.nguoi_xu_ly_chinh:
            if not doc.detail2.filtered(lambda l: l.nguoi_nhap_y_kien == user):
                lines_to_create.append({
                    'office_document_internal_id': doc.id,
        ...
        if lines_to_create:
            self.env['office.document.internal.detail2'].create(lines_to_create)
...
class ButPhe(models.TransientModel):
    _name = 'office.document.internal.but.phe'
    ...
    def but_phe(self):
        ...
        self.env['office.document.internal.detail1'].create({
            'office_document_internal_id': doc.id,
        ...
        if self.tai_lieu_kem:
            self.env['ir.attachment'].create({
                ...
                'res_model': 'office.document.internal',
                'res_id': doc.id,
            })
...
class OfficeDocumentInternalDetail1(models.Model):
    _name = 'office.document.internal.detail1'
    ...
    office_document_internal_id = fields.Many2one('office.document.internal')
...
class OfficeDocumentInternalDetail2(models.Model):
    _name = 'office.document.internal.detail2'
    ...
    def action_open_document(self):
        self.ensure_one()
        user = self.env.user
        details = self.office_document_internal_id.detail2.filtered(lambda d: d.nguoi_nhap_y_kien.id == user.id)
        ...
        return {
            ...
            'res_model': 'office.document.internal',
            'view_mode': 'form',
            'res_id': self.office_document_internal_id.id,
            'target': 'current',
        }
...
class OfficeDocumentInternalDetail3(models.Model):
    _name = 'office.document.internal.detail3'
...
class OfficeDocumentInternal(models.Model):
    _name = 'office.document.internal'
    _description = 'Quản lý công văn nội bộ'
    _rec_name = 'trich_yeu'
    ...
    dv_xu_ly_chinh = fields.Many2many(
        'hr.department',
        'office_document_internal_dv_xu_ly_chinh_rel',
        'internal_id',
        'department_id',
        string='Đơn vị xử lý chính')
    dv_dong_xu_ly = fields.Many2many(
        'hr.department',
        'office_document_internal_dv_dong_xu_ly_rel',
        'internal_id',
        'department_id',
        string='Đơn vị đồng xử lý'
    )
    ...
    nguoi_xu_ly_chinh = fields.Many2many(
        'res.users',
        'office_document_internal_nguoi_xu_ly_chinh_rel',
        'internal_id',
        'user_id',
        string='Người xử lý chính')
    nguoi_dong_xu_ly = fields.Many2many(
        'res.users',
        'office_document_internal_nguoi_dong_xu_ly_rel',
        'internal_id',
        'user_id',
        string='Người đồng xử lý'
    )
    ...
    detail1 = fields.One2many('office.document.internal.detail1', 'office_document_internal_id', string='Ý KIẾN CHỈ ĐẠO VÀ XỬ LÝ')
    detail2 = fields.One2many('office.document.internal.detail2', 'office_document_internal_id', string='Ý KIẾN CẤP LÃNH ĐẠO')
    detail3 = fields.One2many('office.document.internal.detail3', 'office_document_internal_id', string='XỬ LÝ VĂN BẢN CỦA BAN/PHÒNG')
    ...
    def phan_phat(self):
        return {
            ...
            'res_model': 'office.document.internal.phan.phat',
            'target': 'new',
            'context': {
                'footer': False
            }
    ...
    def but_phe_action(self):
        return {
            ...
            'res_model': 'office.document.internal.but.phe',
            'target': 'new'
        }
    ...
    def create(self, vals):
        ...
        return super(OfficeDocumentInternal, self).create(vals)
    ...
    def write(self, vals):
        if 'phan_loai_van_ban' in vals:
            for record in self:
                ...
                super(OfficeDocumentInternal, record).write(new_vals)
            return True
        else:
            return super(OfficeDocumentInternal, self).write(vals)
