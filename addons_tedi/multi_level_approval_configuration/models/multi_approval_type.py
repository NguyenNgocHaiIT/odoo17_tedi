...
Thanks,
    ...
    @api.constrains(
        "approve_python_code", "refuse_python_code", "auto_request_follower_python_code"
    )
...
for rec in self:
  rec['{f_name}'] = rec.env['multi.approval.type'].compute_need_approval(rec)
    ...
    def compute_need_approval(self, rec):
        ...
        if not rec.exists() or isinstance(rec.id, models.NewId) or not dmain:
            return False
    ...
    def exec_func(self, python_code="", eval_context=None, return_val=False):
        try:
            eval_res = safe_eval(
                python_code.strip(),
                eval_context,
                mode=return_val and "eval" or "exec",
                nocopy=True,
...
"""
    ...
    def check_rule(self, records, vals):
        ...
        for rec in records:
            ...
            if constraint_field_names and set(constraint_field_names) & set(vals):
                raise UserError(self._make_err_msg(rec.x_review_result == "refused"))
        return True
    ...
    def _get_type_potential(self, model_name, res_id):
        ...
        record = self.env[model_name].browse(res_id)
        if record.exists() and hasattr(record, potential_fn):
    ...
    def _send_request_auto(self):
        ...
        if not dmain:
            return False
        dmain = expression.AND([dmain, [("x_has_request_approval", "=", False)]])
        try:
